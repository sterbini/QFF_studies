...
line.get_s_position(f'tctpv.4r1.b2')-line.get_s_position(f'ip1')
...
line.element_refs['bbwc.t.4r1.b2'].yma = line.vars['d_wire_ip1.b2']

line.element_refs['bbwc.b.4r1.b2'].current = line.vars['i_wire_ip1.b2']
line.element_refs['bbwc.b.4r1.b2'].yma = -line.vars['d_wire_ip1.b2']

line.element_refs['bbwc.e.4r5.b2'].current = line.vars['i_wire_ip5.b2']
line.element_refs['bbwc.e.4r5.b2'].xma = line.vars['d_wire_ip5.b2']

line.element_refs['bbwc.i.4r5.b2'].current = line.vars['i_wire_ip5.b2']
line.element_refs['bbwc.i.4r5.b2'].xma = -line.vars['d_wire_ip5.b2']
...
# %%
# For reference
[ii for ii in collider.lhcb2.element_names if ('mqy.4l1.b2' in ii) ]


# %%
plt.plot(tw_b2.rows['e.ds.r1.b2':'s.ds.l1.b2']['s'] - tw_b2.rows['ip1']['s'],
        tw_b2.rows['e.ds.r1.b2':'s.ds.l1.b2']['betx'], label='betx')

plt.plot(tw_b2.rows['e.ds.r1.b2':'s.ds.l1.b2']['s']- tw_b2.rows['ip1']['s'],
        tw_b2.rows['e.ds.r1.b2':'s.ds.l1.b2']['bety'], label='bety')

# vertical line at the IP
plt.axvline(x=tw_b2['s','tctpv.4r1.b2']- tw_b2.rows['ip1']['s'],
             color='r', 
             linestyle='--')
# 
plt.axvline(x=tw_b2['s','bbwc.b.4r1.b2']- tw_b2.rows['ip1']['s'],
...
plt.axvline(x=tw_b2['s','mqy.4r1.b2']- tw_b2.rows['ip1']['s'],
             color='m', 
             linestyle='--',
             label='mqy.4r1.b2')

plt.axvline(x=tw_b2['s','mqy.4l1.b2']- tw_b2.rows['ip1']['s'],
             #dark green
             color='m', 
             linestyle='-.',
             label='mqy.4l1.b2')
...
plt.axvline(x=tw_b2['s','mqml.6r1.b2']- tw_b2.rows['ip1']['s'],
             color='k', 
             linestyle='--',
             label='mqml.6r1.b2')

plt.axvline(x=tw_b2['s','mqml.6l1.b2']- tw_b2.rows['ip1']['s'],
             color='k', 
             linestyle='-.',
             label='mqml.6l1.b2')
...
plt.title('Effect of the wires at IP1 for B2 (350~A, 8$\sigma$)')
plt.ylim(-.16, 0.16)
plt.grid()
...
plt.title('Tune shift due to the wires at IP1 for B2')
plt.ylim(-.01, 0.021)
...
plt.xlabel('s [m]')
plt.ylabel('relative $\\beta$-beating')
plt.title('Effect of the wires at IP5 for B2 (350 A, 8$\sigma$)')
plt.ylim(-.16, 0.16)
plt.grid()
...
plt.xlabel('Current [A]')
plt.ylabel('Tune shift [-]')
plt.title('Tune shift due to the wires at IP5 for B2')
plt.ylim(-.01, 0.021)
...
limits_dict = { 'kq5.l1b2': ['kmin_mqml_4.5k','kmax_mqml_4.5k'],
                ...
                'kq7.l1b2': ['kmin_mqm_4.5k','kmax_mqm_4.5k'],
                'kq7.r1b2': ['kmin_mqm_4.5k','kmax_mqm_4.5k'],
                ...
                'kq7.l5b2': ['kmin_mqm_4.5k','kmax_mqm_4.5k'],
                'kq7.r5b2': ['kmin_mqm_4.5k','kmax_mqm_4.5k'],
...
tw_b2[['betx','alfx','bety','alfy'],'ip1']
tw_b2[['betx','alfx','bety','alfy'],'ip5']
tw_b2 = line.twiss(method='4d')

print('Before matching')
print(tw_b2.qx, tw_b2.qy)

variables_list = ['kq5.l1b2_delta', 'kq5.r1b2_delta']
opt = line.match(
...
print(tw_b2.qx, tw_b2.qy)
# %%
plt.plot(tw_b2.s, (tw_b2.betx-betx_0)/betx_0, label='betx')
plt.plot(tw_b2.s, (tw_b2.bety-bety_0)/bety_0, label='betx')
...
for ii in [1]:
        ...
        plt.plot(tw_b2.s-s0, (tw_b2.betx-betx_0)/betx_0, '.-', label='betx')
        plt.plot(tw_b2.s-s0, (tw_b2.bety-bety_0)/bety_0, '.-', label='betx')
        plt.xlim(-5,5)
        plt.grid()
# %%
print('For 350 A and 8 sigma_y at tctpv.4r1.b2\n')
...
variables_list = ['kq4.l5b2_delta', 'kq4.r5b2_delta']
opt = line.match(
    solve=False, # <- prepare the match without running it
    method='4d',
    vary=xt.VaryList(variables_list, limits=(-.001, 0.001),  step=1e-7),
...
# %%
plt.plot(tw_b2.s, (tw_b2.betx-betx_0)/betx_0, label='$\Delta\\beta_x$-beating')
plt.plot(tw_b2.s, (tw_b2.bety-bety_0)/bety_0, label='$\Delta\\beta_x$-beating')
# set xticks only at the IPs
plt.legend()
plt.title('Wires at IP5@350 A, TCTH.4R5.B2@8$\sigma$, Q4 corrected')
plt.xticks([tw_b2['s','ip1'], tw_b2['s','ip2'], tw_b2['s','ip5'], tw_b2['s','ip8']], ['IP1', 'IP2', 'IP5', 'IP8'])
# %%
s0 = tw_b2['s','ip1']
plt.plot(tw_b2.s-s0, (tw_b2.betx-betx_0)/betx_0, '.-', label='$\Delta\\beta_x$-beating')
plt.plot(tw_b2.s-s0, (tw_b2.bety-bety_0)/bety_0, '.-', label='$\Delta\\beta_y$-beating')
plt.xlim(-5,5)
plt.grid()
# %%
print('For 350 A and 8 sigma_x at tctph.4r5.b2')
print_k_summary(my_k_list)
...
variables_list = ['kq5.r1b2_delta', 'kq6.r1b2_delta']
opt = line.match(
    solve=False, # <- prepare the match without running it
    method='4d',
    vary=xt.VaryList(variables_list, limits=(-.001, 0.001),  step=1e-7),
...
plt.plot(tw_b2.s, (tw_b2.betx-tw_ref.betx)/tw_ref.betx, label='betx')
plt.plot(tw_b2.s, (tw_b2.bety-tw_ref.bety)/tw_ref.bety, label='betx')
...
for ii in [1]:
        plt.figure()
        s0 = tw_b2['s',f'ip{ii}']
        plt.plot(tw_b2.s-s0, (tw_b2.betx-tw_ref.betx)/tw_ref.betx, '.-', label='betx')
        plt.plot(tw_b2.s-s0, (tw_b2.bety-tw_ref.bety)/tw_ref.bety, '.-', label='betx')
        plt.xlim(-700,5)
        plt.grid()
        plt.figure()
        plt.plot(tw_b2.s-s0, tw_b2.betx, label='betx')
        plt.plot(tw_b2.s-s0, tw_b2.bety, label='bety')
        plt.grid()
        plt.xlim(-700,5)
        plt.figure()
        plt.plot(tw_b2.s-s0, tw_b2.dx, '.-', label='dx')
        plt.plot(tw_b2.s-s0, tw_b2.dy, '.-', label='dy')
        plt.plot(tw_b2.s-s0, tw_ref.dx, 'sr', label='dx')
        plt.plot(tw_b2.s-s0, tw_ref.dy, 'sr', label='dy')
        plt.xlim(-700,5)
...
for ii in [my_ip]:
        ...
        element_list = line.element_names[line.element_names.index('e.ds.r1.b2'):line.element_names.index('ip1')+1]
